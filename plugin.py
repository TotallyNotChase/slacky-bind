...
def bindcmd(**payload):
    data, channel_id, user, timestamp, web_client, text, text_split = cmd_setup('bind', **payload)
    bot.message_count += 1
    if len(text_split) < 2:
        try:
...
def paste(**payload):
    data, channel_id, user, timestamp, web_client, text, text_split = cmd_setup('ps', **payload)
